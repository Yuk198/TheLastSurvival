...
class Controls_Handler():
    ...
    def navigate_menu(self, actions):
        ...
        if actions["Jump"]: self.curr_index = (self.curr_index + 1) % (len(self.save_file["controls"][str(self.curr_block)]) + 1)
        if actions["Shoot"]: self.curr_index = (self.curr_index - 1) % (len(self.save_file["controls"][str(self.curr_block)]) + 1)
        ...
        # Handle Selection
        if actions["Action1"] or actions["Start"] or actions["Character"]:
