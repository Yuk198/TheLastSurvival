...
class Soldier(pygame.sprite.Sprite):
	...
	def move(self, moving_left, moving_right):
		...
		if self.char_type == 'player':
			if (self.rect.right > Screen.Resolution.SCREEN_WIDTH - Var.SCROLL_THRESH and Var.bg_scroll < (world.level_length * Var.TILE_SIZE) - Screen.Resolution.SCREEN_WIDTH)\
		...
		if self.alive and player.alive:
			...
			if self.vision.colliderect(player.rect):
				...
				if self.idling == False:
					...
					if self.move_counter > Var.TILE_SIZE:
						self.direction *= -1
						self.move_counter *= -1
				else:
		...
		for y, row in enumerate(data):
			for x, tile in enumerate(row):
				if tile >= 0:
					...
					img_rect.x = x * Var.TILE_SIZE
					img_rect.y = y * Var.TILE_SIZE
					tile_data = (img, img_rect)
					if tile >= 0 and tile <= 8:
						self.obstacle_list.append(tile_data)
					elif tile >= 9 and tile <= 10:
						water = Obts.Water(img, x * Var.TILE_SIZE, y * Var.TILE_SIZE)
						InitAssets.SpriteGr.water_group.add(water)
					elif tile >= 11 and tile <= 14:
						decoration = Obts.Decoration(img, x * Var.TILE_SIZE, y * Var.TILE_SIZE)
						InitAssets.SpriteGr.decoration_group.add(decoration)
					elif tile == 15:#create player
						player = Soldier('player', x * Var.TILE_SIZE, y * Var.TILE_SIZE, 1.65, 5, 20, 5)
						health_bar = Screen.HealthBar(10, 10, player.health, player.health)
					elif tile == 16:#create enemies
						enemy = Soldier('enemy', x * Var.TILE_SIZE, y * Var.TILE_SIZE, 1.65, 2, 20, 0)
						InitAssets.SpriteGr.enemy_group.add(enemy)
					elif tile == 17:#create ammo box
						item_box = ItemBox('Ammo', x * Var.TILE_SIZE, y * Var.TILE_SIZE)
						InitAssets.SpriteGr.item_box_group.add(item_box)
					elif tile == 18:#create grenade box
						item_box = ItemBox('Grenade', x * Var.TILE_SIZE, y * Var.TILE_SIZE)
						InitAssets.SpriteGr.item_box_group.add(item_box)
					elif tile == 19:#create health box
						item_box = ItemBox('Health', x * Var.TILE_SIZE, y * Var.TILE_SIZE)
						InitAssets.SpriteGr.item_box_group.add(item_box)
					elif tile == 20:#create exit
						exit = InitAssets.Exit(img, x * Var.TILE_SIZE, y * Var.TILE_SIZE)
...
class ItemBox(pygame.sprite.Sprite):
	def __init__(self, item_type, x, y):
		...
		self.rect = self.image.get_rect()
		self.rect.midtop = (x + Var.TILE_SIZE // 2, y + (Var.TILE_SIZE - self.image.get_height()))
...
class Grenade(pygame.sprite.Sprite):
	...
	def update(self):
		...
		if self.timer <= 0:
			...
			if abs(self.rect.centerx - player.rect.centerx) < Var.TILE_SIZE * 2 and \
				abs(self.rect.centery - player.rect.centery) <  Var.TILE_SIZE * 2:
				player.health -= 50
			for enemy in InitAssets.SpriteGr.enemy_group:
				if abs(self.rect.centerx - enemy.rect.centerx) <  Var.TILE_SIZE * 2 and \
					abs(self.rect.centery - enemy.rect.centery) <  Var.TILE_SIZE * 2:
					enemy.health -= 50
...
with open(f'Source/level{ Var.level}_data.csv', newline='') as csvfile:
